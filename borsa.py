...
# ── Main ──────────────────────────────────────────────────────────────────────
def main():
    # Controlla che sia l'orario giusto (18:00–19:30 ora italiana)
    # Evita doppie esecuzioni quando entrambi i cron di GitHub scattano
    ora_it = datetime.now(ROME_TZ).time()
    from datetime import time as dtime
    if not (dtime(18, 0) <= ora_it <= dtime(19, 30)):
        print(f"Orario attuale {ora_it.strftime('%H:%M')} fuori dalla finestra TAH (18:00-19:30) — skip.")
        return
