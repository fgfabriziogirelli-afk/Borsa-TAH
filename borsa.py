"""
VERSIONE CORRETTA - borsa.py
URL: /borsa/azioni/mercato-serale/contratti.html
"""
import os, re, time, requests, pytz
from bs4 import BeautifulSoup
from datetime import datetime

print(">>> AVVIO borsa.py - URL CORRETTO <<<")
...
URL_CORRETTO = (
    BASE_URL
    + "/borsa/azioni/mercato-serale/contratti.html"
    + "?isin={isin}&mic=MTAH&HH=18&MM=00&lang=it"
)
...
def main():
    for titolo in TITOLI:
        isin = titolo["isin"]
        url  = URL_CORRETTO.format(isin=isin)
        ...
        try:
            ...
            if not contracts:
                send_telegram(
                    f"⚠️ <b>{titolo['nome']}</b>\n"
                    f"📅 {now}\n"
                    f"Nessun contratto 18:00-18:05 trovato.\n"
                    f"(Borsa chiusa o fuori orario TAH)"
                )
            else:
                prezzi = []
                qtot = 0
                ...
                msg = (
                    ...
                    f"{sep}\n"
                    f"<b>Contratti 18:00-18:05 ({len(contracts)})</b>\n"
