...
def fetch_contracts_tah(isin: str) -> tuple[list[dict], dict]:
    """
    Scarica TUTTI i contratti TAH del giorno (senza filtro orario)
    e filtra in Python solo quelli delle 18:00:xx.
    Così funziona anche se GitHub esegue in ritardo.
    """
    url = (
        f"{BASE_URL}/borsa/azioni/mercato-serale/contratti.html"
        f"?isin={isin}&mic=MTAH&lang=it"
    ...
    contracts_18 = [
        ...
        if c["ora"].replace(",", ":").replace(".", ":")[:5] == "18:00"
    ]
    return contracts_18, summary
